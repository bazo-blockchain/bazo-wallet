	...
	<div class="container">
		...
		<div v-if="!isLoading">
			<div v-if="accountDetail">
						...
						<tr>
							...
							<td>
								<span v-if="accountDetail.account.channelTransaction">
									{{ $t('adminAccountsDetail.seeBelow') }} <i class="fa fa-arrow-down"></i>
								</span>
								<span v-else>
									<i class="fa fa-minus"></i>
								</span>
				...
				<div v-if="accountDetail.account.channelTransaction">
					<h2 class="display-7 channel-transaction-title">{{ $t('adminAccountsDetail.channelTransaction.title') }}</h2>
					<div class="row">
						<div class="col-md-6">
							<h3 class="display-8">{{ $t('adminAccountsDetail.channelTransaction.inputTitle') }}</h3>
							<table class="table table-striped">
								<tr>
									<th>Hash</th>
									<th>Index</th>
								...
								<tr v-for="input in channelTransaction.ins">
									<td>{{ input.hash }}</td>
									<td>{{ input.index }}</td>
									<td>{{ input.script }}</td>
						...
						<div class="col-md-6">
							<h3 class="display-8">{{ $t('adminAccountsDetail.channelTransaction.outputTitle') }}</h3>
							<table class="table table-striped">
								<tr>
									<th>Script</th>
									<th>Value</th>
								</tr>
								<tr v-for="output in channelTransaction.outs">
									<td>{{ output.script }}</td>
									<td>{{ output.value }}</td>
								</tr>
							</table>
...
export default {
	...
	computed: {
		channelTransaction: function () {
			// TODO use commented code for bitcoin transaction
			// make sure to also remove the fake channel transaction in load data

			if (window.bitcoin) {
				return window.bitcoin.Transaction.fromHex('');
			} else {
				return {
					ins: [],
					outs: []
